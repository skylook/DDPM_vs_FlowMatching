...
$$
\frac{d}{dt}E[h(X_t)] = E[\nabla h(X_t)^T \dot{X_t}] = E[\nabla h(X_t)^T v_t^*(X_t)] \tag{2}
$$

其中 $v_t^*(X_t) = E[\dot{X_t}|X_t] = E[X_1 - X_0|X_t] = E[X_1 - X_0|X_t=x]$ 是条件期望形式的向量场，与 Definition 3.1 中的 $v_t^*(x)$ 完全一致。
...
$$
\int_{\mathbb{R}^d} h(x)\nabla_x \cdot (v_t^*(x) p_t^X(x))dx = -\int_{\mathbb{R}^d} \nabla_x h(x)^T (v_t^*(x) p_t^X(x))dx \tag{5}
$$
...
$$
\frac{\partial}{\partial t} p_t^Z(z) + \nabla_z \cdot (v_t^*(z) p_t^Z(z)) = 0 \tag{8}
$$

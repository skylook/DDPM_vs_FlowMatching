...
根据泰勒展开的逆向应用，这等价于：
   - 设 $\Delta X = \Delta t\mathbb{E}_{X_0|X_t}\left[\frac{\partial \varphi_t(X_0,X_t)}{\partial t}\right]$
   - 则原式可以理解为 $\phi(X_t + \Delta X)$ 在 $X_t$ 处的泰勒展开
$$
...
$$
X_{t+\Delta t} \approx X_t + \Delta t\mathbb{E}_{X_0|X_t}\left[\frac{\partial \varphi_t(X_0,X_t)}{\partial t}\right] \tag{11}
$$
...
根据期望的性质，对于任意随机变量 $Y$，有：

$$
\mathbb{E}[Y] = \arg\min_{\mu} \mathbb{E}[\|Y-\mu\|^2] \tag{14}
$$

将这一性质应用到我们的问题中，令 $Y = \frac{\partial \varphi_t}{\partial t} = X_1 - X_0$ 和 $\mu = v(X_t,t)$，则：

这表明条件期望 $\mathbb{E}[Y] = \mathbb{E}_{X_0|X_t}[\frac{\partial \varphi_t}{\partial t}] = \mathbb{E}[X_1 - X_0 | X_t]$ 正是优化目标：

$$
\mathcal{L} = \min_{v} \mathbb{E}_{X_0\sim \pi_0, X_1\sim \pi_1} \left[\|v_\theta(X_t,t) - \frac{\partial}{\partial t}\varphi_t(X_0,X_1)\|^2\right] \tag{15}
$$
...
$$
v_t^*(x) = \mathbb{E}[X_1 - X_0 \mid X_t=x] \tag{16}
$$

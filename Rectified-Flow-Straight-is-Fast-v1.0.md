> 传统生成模型（如 DDPM）长期面临效率与质量的权衡困境：扩散模型虽然生成质量高，但需要多步迭代过程，计算效率较低；而流模型理论上可以一步生成，但由于轨迹设计复杂，实际应用效果往往受限。
>
> Rectified Flow 通过学习直线轨迹来连接噪声分布和数据分布，"矫正"（Rectifying）插值过程以简化生成路径。这种方法将原本可能弯曲复杂的轨迹转化为直线，使得生成过程更加直接高效。
>
> 本文主要翻译自英文论文 [Rectified Flow: Straight Is Fast](https://arxiv.org/abs/2303.00593) 及官方博客 [Rectified Flow](https://rectifiedflow.github.io/blog/2024/intro/)。将围绕 Rectified Flow 展开，结构如下：
>
> 1. 理论基础：概述介绍 DDPM 和 Rectified Flow 的问题定义，介绍 Rectified Flow的数学原理和与现有方法的关系
> 2. Rectified Flow 技术实现：详述如何设计和训练直线轨迹模型
> 3. Reflow 迭代：探讨如何通过迭代优化进一步提升轨迹直线性
> 4. 整体流程与应用：回顾整体流程图并展示在图像生成等任务中的效果
> 5. 相关理论推导：介绍文中比较重要的两个定理的推导思路
...
1. 原理复杂，用到了类似随机微分方程（SDE）这类的过程，相对来说理解透彻原理是比较复杂的（一看头就大）。
...
$$
Z_t = Z_0 + \int_0^t v_s^*(Z_s)\mathrm{d}s, \quad \forall t \in [0,1], \quad Z_0 = X_0 \tag{2}

$$

其中 $\dot{Z}_t = \dfrac{\mathrm{d}Z_t}{\mathrm{d}t}$ 表示时间导数，而速度场 $v_t^*(x) = v^*(x,t)$ 是一个待学习的函数，其目的是确保从 $Z_0 \sim \pi_0$ 出发时，通过公式 (2) 积分到 $Z_1$ 能够遵循目标分布 $Z_1 \sim \pi_1$。在这种情况下，我们称随机过程 $Z=\{Z_t\}$ 提供了从 $\pi_0$ 到 $\pi_1$ 的（ODE）传输。
...
$$
\hat{Z}_{t+\epsilon} = \hat{Z}_t + \epsilon\, v_t^*(\hat{Z}_t), \quad \forall t \in \{0, \epsilon, 2\epsilon, \dots, 1\} \tag{3}
...
***图5：** Lady Windermere's fan 图示了欧拉方法轨迹的误差累积情况：不同初始点的轨迹随着时间偏离真实解曲线。*

![](assets/20250317_180020_best-flow-straight.jpg)

***图6：** 理想情况下 ODE 轨迹完全为直线时，欧拉法可实现零离散化误差。*

理想情况是当 ODE 轨迹完全为直线时，欧拉法可实现**零离散化误差**，而不受步长选择的影响。在这种情况下，经时间重参数化后，ODE 应满足：

$$
Z_t = t\,Z_1 + (1-t)\,Z_0, \quad \implies \quad \dot{Z}_t = \frac{\mathrm{d}Z_t}{\mathrm{d}t} = Z_1 - Z_0 \tag{4}

$$

这些 ODE 被称为**直线传输**，它们能够实现可在一步内模拟的**快速**生成模型（参见公式 (4)）。我们将所得的对 $ (Z_0,Z_1) $ 称为 $ \pi_0 $ 与 $ \pi_1 $ 的直线耦合。尽管在实际中可能无法达到完美直线性，但我们可以尽量使 ODE 轨迹直，从而最大化计算效率。

> **耦合（Coupling）的作用**
> 在概率论中，两个分布 $ \pi_0 $ 和 $ \pi_1 $ 的**耦合**是指它们的联合概率分布 $ \gamma(x_0, x_1) $，满足：
>
> $$
> \int \gamma(x_0, x_1) dx_1 = \pi_0(x_0), \quad \int \gamma(x_0, x_1) dx_0 = \pi_1(x_1) \tag{5}
>
> $$
>
...
* **速度场估计：**
  ...
  $$
  \mathcal{L}=\min_v \int_0^1 \mathbb{E}\Bigl[\|\dot{X}_t - v_t^*(X_t)\|^2\Bigr] dt. \tag{9}
  ...
  >
  > 在线性插值的情况下，$\dot{X}_t = X_1 - X_0$，是一个常数（不依赖于 $t$）。
  >

  那我们整个 Rectified Flow 的训练方法就定义完了，看起来非常简单。但很明显我们会想到下面的**两个问题**：

  1. 🚀️ 这个速度场 $v_t^*(x)$ 的最优解是什么？

  在后面的证明中，我们会经过推导得到这一优化问题的最优解为条件均值：

  $$
  v_t^*(x) = \mathbb{E}\bigl[\dot{X}_t \mid X_t = x\bigr]. \tag{11}

  $$

  2. 🚀️ 通过这个速度场 $v_t^*(x)$ 能够保证生成的分布 $Z_t$ 与目标样本分布 $X_t$ 一致吗？

  从最优解看来，$v_t^*(x)$ 肯定是不会交叉的，且也不可能保证和原始样本一样是直线差值。

  ![](assets/20250310_144416_retified-flow-show.gif)

  ***图8：** Rectified Flow 的三种轨迹示意图。(a) $X_t$ 线性插值轨迹；(b) 通过 $v_t^*(x)$ 重构的轨迹；(c) 理想的直线轨迹。蓝色和绿色表示不同模式。*

  * 图8(a) 展示了 $X_t$ 线性插值的轨迹（直线，且可能存在相交）
  * 图8(b) 展示了通过 $v_t^*(x)$ 的速度场重构的 Rectified Flow 轨迹（不相交，也不一定为直线）
  * 图8(c) 是我们希望实现的理想的一一对应的直线路径，那在这种情况下因为是直线我们可以 "一步生成"。

  看起来轨迹都不一样，那么 $X_t$ 和 $Z_t$ 的分布是一样的吗（即 $p(x_t) = p(z_t)$）？

  ![](assets/20250319_112204_rectified_flow_vector_field.svg)

  ***图9：** 速度场 $v_t^*(x)$ 的直观解释。在每个点处，速度场取所有经过该点的轨迹速度的条件均值。*

  **直观解释：**

  - 在插值过程 $\{X_t\}$ 中，不同轨迹可能相交，因而在某一点 $X_t$ 处可能得到多个不同的 $\dot{X}_t$ 值（参见图8a）。
  - 相比之下，在由 $\dot{Z}_t = v_t^*(Z_t)$ 定义的 ODE 中，每个点 $Z_t$ 的更新方向是唯一确定的，这避免了交叉后方向分叉的情况。
  - 所以，在交叉点处，ODE 通过采用条件均值 $v_t^*(x)$ 来"去随机化"更新方向，从而将各插值轨迹重构为不交叉的路径（参见图8b）。
  - 由于 ODE 轨迹 $\{Z_t\}$ 不能相交，它们必须在潜在的交叉点处弯曲，以"重连"原始插值路径并避免交叉。

  实际上，可以证明在任意时刻 $t$，$X_t$ 和 $Z_t$ 的分布是相同的。这一性质称为边际保持性（Marginal Preservation），可以用以下公式表示：

  $$
  p_{Z_t}(z) = p_{X_t}(z), \quad \forall t \in [0,1] \tag{12}

  $$

## Reflow

尽管 Rectified Flow 倾向于生成更直的轨迹，但这些轨迹并不完全是直线。正如图8(b)所示，在插值轨迹的交叉点处，流仍可能发生转弯。那么，我们如何进一步提升流，使得轨迹更直，从而加速推理？
...
$$
(Z_0^{k+1}, Z_1^{k+1}) = \texttt{Rectify}(Z_0^k, Z_1^k), \quad k \geq 0 \tag{13}

$$

其中 $k$ 表示迭代次数。每一次迭代，我们都：

1. 使用当前的起止对 $(Z_0^k, Z_1^k)$ 训练一个新的 Rectified Flow
2. 用这个新的流生成新的起止对 $(Z_0^{k+1}, Z_1^{k+1})$

这个过程会产生一系列 $k$-Rectified Flow，其中：

- 1-Rectified Flow 就是标准的 Rectified Flow
- 2-Rectified Flow 是在 1-Rectified Flow 基础上再次应用整流操作
- 以此类推

![](assets/20250310_112636_reflow-example.jpg)

***图10：** 1-Rectified Flow 与 2-Rectified Flow 的生成效果对比。可以看到 2-Rectified Flow 生成的样本质量更好，且轨迹更直。*

理论上，随着 $k$ 的增加，轨迹会变得越来越直。在实践中，我们发现 $k=2$ 就足以获得显著改进，进一步增加 $k$ 的收益不大。
...
$$
S(Z_t) = \mathbb{E}\left[\int_0^1 \left\|\frac{\mathrm{d} Z_t}{\mathrm{d}t} - (Z_1 - Z_0)\right\|^2 \mathrm{d}t\right] \tag{14}
...
$$
\mathbb{E}_{k \sim \mathrm{Unif}(\{1,\dots,K\})}\Bigl[S(\{Z_t^k\})\Bigr] = \mathcal{O}\Bigl(\frac{1}{K}\Bigr) \tag{15}

$$

这意味着在前 $ K $ 次迭代中，直线性度量的平均值以 $ \mathcal{O}(1/K) $ 的速率衰减（参见公式 (15)）。
...
$$
Z_1 \approx Z_0 + v_\theta(Z_0, 0) \tag{16}

$$

这大大加速了生成过程，可以接近图 8(c) 的理想结果，使得 Rectified Flow 在实际应用中更具吸引力。
...
2. **重复直到满足终止条件**:
   ...
   - **评估直线性**:
     - 计算新流的直线性度量 $S(Z_t^{k+1})$（参见公式 (14)）
     - 如果 $S(Z_t^{k+1})$ 小于阈值或迭代次数达到上限，则停止迭代
   - 更新 $k = k + 1$
3. **最终输出**:

   - 最优速度场 $v_\theta^k$（可用于一步采样 $Z_1 \approx Z_0 + v_\theta^k(Z_0, 0)$（参见公式 (16)））

最终整体流程图如下：

![](figures/Reflow_flow.png)

***图11：** Rectified Flow 和 Reflow 的整体流程图。左侧展示了标准 Rectified Flow 的训练和推理过程，右侧展示了 Reflow 如何通过迭代提升轨迹的直线性。*
...
***图12：** 1-Rectified Flow 与 2-Rectified Flow 的生成效果对比。左：使用标准 Rectified Flow（k=1）生成的样本；右：使用 2-Rectified Flow 生成的样本。可以看到 2-Rectified Flow 不仅生成质量更好，而且能够使用更大的步长进行快速采样。*
